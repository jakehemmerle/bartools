...
export const DEFAULT_USER_PROMPT_TEMPLATE = [
  ...
  "",
  "Choose the closest match from this list of {bottle_count} bottle names; copy the name VERBATIM:",
  "",
  "{possible_bottle_names_text}",
  "",
...
export function buildDefaultPromptTemplate(): ChatPromptTemplate {
  return ChatPromptTemplate.fromMessages([
    ["system", DEFAULT_SYSTEM_PROMPT],
    ["human", DEFAULT_USER_PROMPT_TEMPLATE],
...
export async function renderPromptTemplate(
  ...
  return promptValueToModelInput(
    await prompt.invoke({
      bottle_count: bottleNames.length,
      possible_bottle_names_text: bottleNames.join("\n"),
    }),
  );
}
...
export async function pushDefaultPromptTemplate(promptName: string): Promise<string> {
  await client.pushPrompt(promptName, {
    object: buildDefaultPromptTemplate(),
    description:
