...
Canonical UI rule:
- use 0 through 10 as the visible control labels
- a record value of `fillPercent: 40` corresponds to `4` in the UI
- a record value of `fillPercent: 60` corresponds to `6` in the UI
- do not show both `4` and `40%` as competing primary fill labels on the same screen
...
Canonical `ReportProgress` example:
